...
import { getRequestsPath, getRequestPublicPath } from "@/lib/routes/marketplace";
...
export async function generateMetadata({ params }: PageProps) {
  const { country, requestSlug } = await params;

  const { data } = await supabase
    .from("service_requests")
    .select(
      "public_slug, city, state, country_code, public_description, subcategory_slug, category_slug, market_slug, lead_price_credits, lead_price_fixas, index_status"
    )
  ...
  const canonicalCountry = data.country_code || country;
  const canonicalPath = getRequestPublicPath(data.public_slug, canonicalCountry);
...
export default async function RequestPage({ params }: PageProps) {
  ...
  const requestCountry = request.country_code || country;
  const requestsPath = getRequestsPath(requestCountry);
  ...
  const breadcrumbJsonLd = getBreadcrumbJsonLd([
    { name: "Home", url: "/" },
    { name: "Requests", url: requestsPath },
    { name: jobTitle, url: publicRequestPath },
  ]);

  const breadcrumbs = [
    { label: "Home", href: "/" },
    { label: "Requests", href: requestsPath },
    { label: jobTitle },
  ];
